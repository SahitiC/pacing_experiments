...
STATES_NO = 30+1  # one extra state for completing nothing
...
REWARD_UNIT = 0.5  # reward per unit
BETA = 10  # softmax beta for diff-disc model
N_TRIALS = 40  # no. of trajectories
REWARD_SHIRK = 0.1
EFFORT_WORK = -0.3

DISCOUNT_FACTOR_REWARD = 0.96
DISCOUNT_FACTOR_COST = 0.62
...
if __name__ == "__main__":
    ...
    # ax1.legend(bbox_to_anchor=(0.5, 1.25), ncol=4, frameon=False,
    #            fontsize=18, loc='upper center', columnspacing=0.5)
    ax1.set_title(
    ...
    for reward_unit in rewards_unit:
        ...
        helper.plot_trajectories(trajectories, 'black', 2, 0.5,
                                 number_samples=10, ylim=STATES_NO,
                                 xticks=[0, 7, 15], yticks=[0, 10, 20])
